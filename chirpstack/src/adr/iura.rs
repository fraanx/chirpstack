...
impl Handler for Algorithm {
    fn get_name(&self) -> String {
        "Iura ADR algorithm (iura-xss only)".to_string()
    }
...
///////////////////////
        ...
        //force to initial values for test
       ...
       // let n_step = (snr_margin / 4.0) as isize;
        ...
        let ch_freqs = [1571000000.0,1571240000.0,]; // [1101000000.0,1101500000.0,1102000000.0];
        // let kl_buclnb_offset = -50000000.0;     // K-band

        // initialize SampleProxy RX Frequency 
        // let mut sp_rxfreq=1000000000.0;

        println!("[IURA-ADR] Channel 1 freq ={:?}", ch_freqs[0]);
        println!("[IURA-ADR] Channel 2 freq ={:?}", ch_freqs[1]);
        ...
        for ch_idx in ch_mask_indexes {
            ...
            if ch_idx < 2 {          
                resp.ch_mask[(ch_idx % 16) as usize] = true;
                println!("[IURA-ADR] Initing Resp Ch Mask idx {:?} to true", ch_idx+1);
            } else{
                resp.ch_mask[(ch_idx % 16) as usize] = false;
                println!("[IURA-ADR] Initing Resp Ch Mask idx {:?} to false", ch_idx+1);
            }   
            println!("[IURA-ADR] Current Resp Ch Mask={:?}", resp.ch_mask);

            // retrieve pkt rate
            println!("Retrieving kpis for channel {:?} ",ch_idx+1);
            let kpis_request = tonic::Request::new(ChannelWindowRequest {
                window_duration: 5000,
                channel_id: ch_idx as u32,
                });
                
            // Send the request and get the response
            let kpis_response = dtclient.channel_kpis_per_window(kpis_request).await?;

            for kpis in kpis_response.into_inner().kpis {
                let stt_stats = kpis.stt_stats;
                println!("STT for Channel {:?} = {:?}", ch_idx+1, stt_stats);
                if stt_stats > stt_thres_high {
                    println!("Disabling channel {:?}", ch_idx+1);
                    resp.ch_mask[(ch_idx % 16) as usize] = false;
                    println!("Resp Ch Mask {:?} = {:?}",ch_idx+1, resp.ch_mask[(ch_idx % 16) as usize]);
                } else{
                    println!("Enabling channel {:?}", ch_idx+1);
                    resp.ch_mask[(ch_idx % 16) as usize] = true;
                    println!("Resp Ch Mask {:?} = {:?}",ch_idx+1, resp.ch_mask[(ch_idx % 16) as usize]);
                }
               }
        ...
        info!(NewDr = resp.dr , "[IURA-ADR] New Dr is");
        info!(NewTxPwrIndex = resp.tx_power_index , "[IURA-ADR] New TxPwrIndex is");
